...
LLM_BASED_ID_ASSIGNMENT = os.getenv("LLM_BASED_ID_ASSIGNMENT", "true").lower() == "true"
...
def create_text_model(temperature: float = 0.1, max_tokens: int = 16000, **kwargs):
    ...
    if not validate_provider_config(provider):
        raise ValueError(f"Provider '{provider}' is not valid. Please check your API key.")
...
def validate_all_configs():
    ...
    if not validate_provider_config(TEXT_LLM_PROVIDER):
        errors.append(f"Text LLM provider '{TEXT_LLM_PROVIDER}' is not valid. Please check your API key.")
    
    if not validate_provider_config(VISION_LLM_PROVIDER):
        errors.append(f"Vision LLM provider '{VISION_LLM_PROVIDER}' is not valid. Please check your API key.")
...
try:
    ...
    logger.warning(f"Configuration validation failed, using default values: {e}") 